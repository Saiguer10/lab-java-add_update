...
No. Usé `PUT` para el doctor, y para le paciente no he sido capaz al final

### ¿Por qué elegiste esa estrategia?
Elegí PUT porque es ideal cuando se necesita reemplazar completamente un recurso, mientras que PATCH es más adecuado para modificar 
solo algunas partes de un recurso sin afectar el resto.
### ¿Ventajas y desventajas?
- `PUT`: Asegura que el recurso sea completamente reemplazado, lo cual es útil para mantener la consistencia, 
        pero puede resultar innecesario cuando solo se quiere modificar un dato.
- `PATCH`: Permite realizar cambios parciales, lo que lo hace más eficiente en ciertas situaciones.

### ¿Coste-beneficio entre PUT y PATCH?
- `PUT` iRequiere enviar todos los campos del recurso, lo que puede ser costoso en términos de 
        rendimiento si solo se necesita cambiar una pequeña parte.

- `PATCH` Es más eficiente cuando solo se actualizan algunos campos, pero puede ser más complicado de implementar
